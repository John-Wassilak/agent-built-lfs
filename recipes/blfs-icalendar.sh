...
# Build backend: hatchling (not installed).
# Runtime deps (via --no-deps, installed as separate steps in this same closure): python-dateutil, tzdata.
...
pip3 install --no-index --find-links dist --no-user icalendar
