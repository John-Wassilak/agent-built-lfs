...
# source : book/blfs-13.1/general/libwacom.html
# title  : libwacom-2.19.1
# The driver supplies unpack/cd/cleanup. Commands below are in-package only.
set -e

# --- block 0 --------------------------------------------------
#   ctx: space required: 17 MB (with tests) Estimated build time: less than 0.1 SBU (with tests)
#   ctx: libwacom Dependencies Required libevdev-1.13.6 and libgudev-238 Recommended
#   ctx: libxml2-2.15.3 Optional Doxygen-1.18.0, git-2.55.0, librsvg-2.62.3, Valgrind-3.27.1
#   ctx: (optional for some tests), and pytest-9.1.1 with python-libevdev and pyudev Installation
...
# --- block 1 --------------------------------------------------
#   ctx: To test the results, issue: ninja test. To run additional tests, install pytest-9.1.1,
