...
# Build backend: setuptools.build_meta but needs setuptools_scm>=8 (not installed).
# Runtime deps (via --no-deps, installed as separate steps in this same closure): click, click-log, icalendar, urwid, pyxdg, pytz, python-dateutil, configobj, tzlocal.
...
pip3 install --no-index --find-links dist --no-user khal
