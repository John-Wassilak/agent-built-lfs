...
# Build backend: in-tree pep517_backend wrapping setuptools, needs expandvars (not installed).
# Runtime deps (via --no-deps, installed as separate steps in this same closure): idna, multidict, propcache.
...
pip3 install --no-index --find-links dist --no-user yarl
