...
# Build backend: setuptools (legacy setup.py).
# Runtime deps (via --no-deps, installed as separate steps in this same closure): click.
...
pip3 install --no-index --find-links dist --no-user click-log
