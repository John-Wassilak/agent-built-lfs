...
# Build backend: setuptools.build_meta but needs the pkgconfig PyPI module (not installed).
# Runtime deps (via --no-deps, installed as separate steps in this same closure): aiohappyeyeballs, aiosignal, attrs, frozenlist, multidict, propcache, yarl.
...
pip3 install --no-index --find-links dist --no-user aiohttp
