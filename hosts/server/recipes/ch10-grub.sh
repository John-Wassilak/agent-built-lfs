...
#   REVIEWED [drop]: New in LFS 13.1 (2026-09-07 bump): `grub-install --target=x86_64-efi --removable`. This host has no EFI System Partition to install to (host.toml's [hardware].boot: BIOS/MBR on sdb) -- block 1's `grub-install /dev/sda --target=i386-pc` right above is the real install for this machine.
...
#   REVIEWED [drop]: New in LFS 13.1, admonition-tagged by the book itself: mounts efivarfs to register a boot entry. No /sys/firmware/efi on a BIOS/MBR boot (confirmed live: this host has no /sys/firmware/efi directory at all).
# mountpoint /sys/firmware/efi/efivars ||
#   mount -v -t efivarfs efivarfs /sys/firmware/efi/efivars

# --- block 4 --------------------------------------------------
#   ctx: Now create a boot entry for the EFI:
#   REVIEWED [drop]: efibootmgr boot-entry registration -- meaningless without EFI variables; see block 3's reason.
...
#   REVIEWED [drop]: Unmounts the efivarfs mounted in block 3 -- nothing to unmount since block 3 is dropped.
