...
# source : book/blfs-13.1/multimedia/wireplumber.html
# title  : Wireplumber-0.5.15
#
# HOST COPY, created 2026-09-21. This is the shared recipes/blfs-wireplumber.sh
# re-read against 13.1, which changed exactly one thing: the versioned docdir
# below is 0.5.15, not 0.5.13. That single version string is why this cannot stay
# in the shared file -- `laptop` is pinned to BLFS 13.0 and still builds 0.5.13
# from it (hosts/laptop/packages.py seq 130.6, labelled "shared recipe"), so
# editing the shared copy in place would silently install a 0.5.15 docdir path
# for a 0.5.13 tree on that machine. The driver exports no version variable to
# recipes, so there is no way to write the rename once and have it be correct for
# both. Everything else on the 13.1 page is byte-identical to 13.0, including the
# Configuring Wireplumber section below.
...
ninja install
mv -v /usr/share/doc/wireplumber /usr/share/doc/wireplumber-0.5.15
