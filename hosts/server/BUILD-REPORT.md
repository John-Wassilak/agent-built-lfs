...
**Re-checked 2026-09-21, merging this host's 13.1 bump back together with `laptop`'s
work.** The conversion stands, and the 14 entries that moved in the bump (glad,
libplacebo, luajit, uchardet, nspr, nss, libarchive, libnotify, startup-notification,
libevent, usbutils, hicolor-icon-theme, and spirv-llvm-translator/libclc at their new
55.8/55.9) are `book()` at their new positions. `firefox` is the exception and stays
`hand()`: the bump gave it `hosts/server/recipes/blfs-firefox.sh`, so its recipe is no
longer the shared file and no other host can reach it. `extract-blfs.py --host laptop
--check` reports zero problems with this resolution and 14 without it.

`state/blfs-plan.json` here is still the bump's own output and lists those 14 as
hand-authored. Regenerating it needs `book/blfs-13.1`, which is not in the tree on
`laptop` -- so `extract-blfs.py --host server` is owed on a machine that has the 13.1
book before this host's plan matches its package list again.
